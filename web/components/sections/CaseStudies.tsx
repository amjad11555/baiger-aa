          ...
          {dict.cases.items.map((item, i) => (
            <Reveal key={item.sector} delay={i * 0.05}>
              <article className="glass grid gap-8 rounded-3xl p-8 md:grid-cols-[minmax(220px,1fr)_2fr] md:p-12">
                <div>
                  <p className="font-mono text-xs tracking-[0.18em] uppercase text-mist">
                    {item.sector}
                  </p>
                  <p className="text-gradient-ember font-display mt-4 text-6xl font-extrabold md:text-7xl">
                    {item.metric}
                  </p>
                  <p className="mt-2 font-mono text-sm text-iris">
                    {item.metricLabel}
                  </p>
                </div>
                <dl className="space-y-6 self-center">
                  <div>
                    <dt className="mb-1.5 font-mono text-xs tracking-[0.18em] uppercase text-iris">
                      {dict.cases.labels.challenge}
                    </dt>
                    <dd className="leading-relaxed text-mist">{item.challenge}</dd>
                  </div>
                  <div>
                    <dt className="mb-1.5 font-mono text-xs tracking-[0.18em] uppercase text-iris">
                      {dict.cases.labels.play}
                    </dt>
                    <dd className="leading-relaxed text-ink">{item.play}</dd>
                  </div>
                </dl>
              </article>
            </Reveal>
