...
function buildContextNote(from) {
  ...
  return (
    `ملاحظة سياقية — ملف العميل (سجلّ سابق، لا يحدّد موضوع المحادثة الحالية): ` +
    `الاسم: ${lead?.name || contact?.name || '—'} | ` +
    `المرحلة: ${lead?.stage || 'new'} | الاهتمام المسجّل سابقًا: ${lead?.interest || '—'} | ` +
    `الميزانية: ${lead?.budget_signal || '—'} | الخطوة التالية: ${lead?.next_action || '—'}. ` +
    `⛔ إن ذكر العميل الآن حاجة أو خدمة مختلفة عن «الاهتمام المسجّل سابقًا»، ` +
    `فالأحدث هو الصحيح: اتبع ما قاله الآن، واستبدل الاهتمام عبر update_lead فورًا. ` +
    `لا تُعِد العميل إلى خدمة قديمة لأنها مسجّلة في ملفه.`
  );
}
